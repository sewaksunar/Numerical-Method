...
This course module covers the fundamental numerical techniques used to find the roots (or zeros) of functions where analytical solutions are difficult or impossible to obtain.
...
The core of the multi-equation Newton-Raphson method is the **Jacobian matrix** (denoted as $[J]$), which consists of the partial derivatives of the equations with respect to each unknown. For two equations, the matrix is:
...
### 1.7.4 The Matrix Update Equation
...
### 1.7.5 Solution via Matrix Inversion (Formal Representation)
