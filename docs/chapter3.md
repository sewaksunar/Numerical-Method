...
Consider the points $(x_{0}, f_{0}), (x_{1}, f_{1}), \cdots, (x_{n}, f_{n})$. We find a polynomial $p_n(x)$ of degree $n$ or less that assumes the values $p_n({x_0}) = f_{0},\quad p_n({x_1}) = f_{1} \quad, \cdots, \quad p_n({x_n}) = f_{n}$.
...
Let $y=f(x)$ take the values $y_0, y_1, y_2, \cdots, y_n$ corresponding to the equally spaced values $x_0, x_1, x_2, \cdots, x_n$ with spacing factor $h$.
...
$$
f(x) = y_{0} + p \Delta y_{0} + \frac{p(p-1)}{2!} \Delta^2 y_{0} + \frac{p(p-1)(p-2)}{3!} \Delta^3 y_{0} + \cdots + \frac{p(p-1) \cdots (p-(n-1))}{n!} \Delta^n y_{0}
$$
...
$$
f(x) = y_{n} + p \nabla y_{n} + \frac{p(p+1)}{2!} \nabla^2 y_{n} + \frac{p(p+1)(p+2)}{3!} \nabla^3 y_{n} + \cdots + \frac{p(p+1) \cdots (p+(n-1))}{n!} \nabla^n y_{n}
$$

where $p = \dfrac{x - x_{n}}{h}$
...
$$\mu \Delta y_{0} = \frac{\Delta y_{0} + \Delta y_{-1}}{2}, \quad \mu \Delta^{3} y_{0} = \frac{\Delta^{3} y_{0} + \Delta^{3} y_{-1}}{2}$$
...
$$\delta y_{1/2} = y_{1} - y_{0}, \quad \delta^{2} y_{1} = y_{2} - 2y_{1} + y_{0}$$
...
Consider a set of $(n+1)$ arbitrarily spaced data $(x_{0}, f_{0}), (x_{1}, f_{1}), \cdots, (x_{n}, f_{n})$. Then Lagrange interpolation polynomial is given by:
...
Consider a set of $(n+1)$ arbitrarily spaced data $(x_{0}, f_{0}), (x_{1}, f_{1}), \cdots, (x_{n}, f_{n})$. Let:

$$p_{n}(x) = a_{0} + a_{1}(x-x_{0}) + a_{2}(x-x_{0})(x-x_{1}) + \cdots + a_{n}(x-x_{0})(x-x_{1}) \cdots (x-x_{n-1})$$

be the polynomial that interpolates the above points. Then, if $p_n({x_0}) = f_{0},\quad p_n({x_1}) = f_{1} \quad, \cdots, \quad p_n({x_n}) = f_{n}$, then:
...
$$
\begin{align}
p_{n}(x_{2}) &= a_{0} + a_{1}(x_{2} - x_{0}) + a_{2}(x_{2}-x_{0})(x_{2}-x_{1}) \\
...
Substituting the values of $a_0, a_1, a_2, \cdots, a_n$, we get Newton's divided difference interpolation polynomial:
...
$$
f[x_0,x_1]=\frac{f_1-f_0}{x_1-x_0}, \quad
f[x_1,x_2]=\frac{f_2-f_1}{x_2-x_1}, \quad
f[x_2,x_3]=\frac{f_3-f_2}{x_3-x_2}
$$

$$
f[x_0,x_1,x_2]=\frac{f[x_1,x_2]-f[x_0,x_1]}{x_2-x_0}, \quad
